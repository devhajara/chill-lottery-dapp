...
const baseUrl = "https://chill-and-win.up.railway.app/api";
...
function AdminPanel() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        // Entries
        const entryRes = await fetch(`${baseUrl}/entries/${data.id}`);
        const entryData = await entryRes.json();
        setEntries(entryData.map((e: any) => e.wallet));

        // Winners
        const winnerRes = await fetch(`${baseUrl}/winners`);
  ...
  const pickWinners = async () => {
    try {
      ...
      const declareRes = await fetch(`${baseUrl}/winner`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
const ManualWinnerSelector = ({ isOpen, onClose }: ManualWinnerSelectorProps) => {
  ...
  useEffect(() => {
    ...
    const fetchLotteryAndEntries = async () => {
      try {
        ...
        const entryRes = await fetch(`${baseUrl}/entries/${lottery.id}`);
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await fetch(`${baseUrl}/declare-winner`, {
        method: "POST",
